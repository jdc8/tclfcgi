...
proc configure {args} {
    ...
    set d [dict create fcgi $fcgi static $static ldir $ldir idir $idir config $config]
    return $d
}
proc Hinstall {} { return "?destination? ?config? ?-fcgi <path>? ?-static? ?-dynamic?\n\tInstall all packages.\n\tdestination = path of package directory, default \[info library\].\n\tconfig = Critcl target to be used\n\t-fcgi <path> = path to libfcgi\n\t-static = link libfcgi statically, default is to link dynamically\n\t-dynamic = link ligfcgi dynamically" }
proc _install {args} {
    global packages
	...
	if {[string length $fcgi]} {
	    lappend rcargs -L [file join $fcgi lib] -I [file join $fcgi include]
	} else {
...
proc Hdebug {} { return "?destination? ?config? ?-fcgi <path>? ?-static? ?-dynamic?\n\tInstall debug builds of all packages.\n\tdestination = path of package directory, default \[info library\].\n\tconfig = Critcl target to be used\n\t-fcgi <path> = path to libfcgi\n\t-static = link libfcgi statically, default is to link dynamically\n\t-dynamic = link libfcgi dynamically" }
proc _debug {args} {
    global packages
    set d [eval configure $args]
    dict with d {}
...
proc _gui {} {
    global INSTALLPATH LIBFCGIPATH BUILDSTATIC
    package require Tk
#    package require widget::scrolledwindow
    ...
    label  .lz -text {libfcgi Path: }
    entry  .ez -textvariable ::LIBFCGIPATH
    button .bz -command [list Browse ::LIBFCGIPATH] -text Browse
    checkbutton .bs -text "Link libfcgi statically" -variable BUILDSTATIC -anchor w
...
proc Install {} {
    global INSTALLPATH LIBFCGIPATH NOTE
	...
	_install $INSTALLPATH -fcgi $LIBFCGIPATH %s
...
proc Debug {} {
    global INSTALLPATH LIBFCGIQPATH
    foreach p {.ei .ez .bi .bz .bs .i .d} {
	$p configure -state disabled
    }

    set fail [catch [format {
	_debug $INSTALLPATH -fcgi $LIBFCGIPATH %s
